...
-(void)setupXHLaunchAd{
    ...
    NSString *imageUrl = [NSString stringWithFormat:@"new_ads/recent/?height=%.f&width=%.f",XScreenH,XScreenW];
    [[MNNetworkTool shareService]requstWithUrl:imageUrl Param:nil Success:^(id responseObj) {
//        XLog(@"responseObj -- %@",responseObj);
        ...
        if (!model.hide_mono_logo) {
            CGFloat adHeight = (CGFloat)model.height / model.width * XScreenW;
            imageAdconfiguration.frame = CGRectMake(0, 0, XScreenW, adHeight);
            UIImageView *adBottomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, adHeight, XScreenW, XScreenH - adHeight)];
